...
When all blocks have been removed, the player can choose between 3 upgrades (common = white, rare = blue, legendary = yellow) and a new level gets generated.

The block's hp and amount of blocks scale with level and higher levels introduce new tiers of blocks with even higher hp.

The player gains score when removing a block which is based on the tier of the block removed, the current level, and a combo multiplier which gets reset when the ball hits the platform again.
...
How to run:
Run the following command in the terminal: java -jar YOURPATH\SirSlimesAdventure.jar
